...
import { COMMENT_RETENTION_DAYS, FEEDBACK_MAX_LEN, FEEDBACK_PAGE_SIZE } from "../lib/site-config.mjs";
import { getSql } from "../lib/db.mjs";

export { COMMENT_RETENTION_DAYS, FEEDBACK_MAX_LEN, FEEDBACK_PAGE_SIZE };
...
export function dailyRateLimitIdentity(req, secret = process.env.RATE_LIMIT_SECRET) {
...
export async function insertFeedback(sql, req, body, version, options = {}) {
  const dailyLimit = options.dailyLimit ?? 2;
  const identity = dailyRateLimitIdentity(req, options.rateLimitSecret);
  ...
  return sql`
    ...
    quota AS (
      ...
      WHERE feedback_rate_limits.post_count < ${dailyLimit}
      RETURNING 1
    )
...
export async function listComments(sql, req, options = {}) {
  const defaultPageSize = options.defaultPageSize ?? FEEDBACK_PAGE_SIZE;
  const maxPageSize = options.maxPageSize ?? 100;
  const url = new URL(req.url || "/", "http://localhost");
  const limit = Math.min(
    Math.max(parsePageNumber(url.searchParams.get("limit"), defaultPageSize), 1),
    maxPageSize
  );
...
export async function handleCommentsRequest(req, res, deps = {}) {
  const getSqlFn = deps.getSql ?? getSql;
  setCors(req, res);
  ...
  if (req.method === "GET") {
    ...
    try {
      const page = await listComments(sql, req, deps);
  ...
  if (req.method === "POST") {
    ...
    if (!body || body.length > (deps.maxBody ?? FEEDBACK_MAX_LEN)) {
    ...
    if (payload.version !== undefined && payload.version !== null) {
      ...
      if (version && version.length > (deps.maxVersion ?? 50)) {
        return json(res, 400, { error: "invalid_version" });
      }
    }
    ...
    try {
      const inserted = await insertFeedback(sql, req, body, version, deps);
      if (inserted === null) {
